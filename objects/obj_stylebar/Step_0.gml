...
if (global.stylethreshold == 3 && global.style > 55)
    global.style = 55
if (global.stylemultiplier > 0 && global.superactive == 1 && (!instance_exists(obj_johnhitstun)))
    global.style -= 0.2
if (global.stylemultiplier <= 0.05 && global.superactive == 1)
{
    obj_player.hypernoise = 0
...
if (obj_player.y < 200)
    alpha = 0.3
else if (!((room == rank_room || room == timesuproom || room == boss_room1 || room == Realtitlescreen || room == characterselect || room == Titlescreen)))
    alpha = 1
index += 0.35
