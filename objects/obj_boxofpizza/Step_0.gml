...
with (obj_player)
{
    ...
    if (other.image_yscale == -1)
    {
        if (((key_up && (!(place_meeting(x, (y - 1), obj_destructibles))) && place_meeting(x, (y - 10), other) && (state == states.normal || state == states.pogo || state == states.jump || state == states.mach1 || state == states.mach2 || state == states.walljump || state == states.mach3 || state == states.Sjumpprep || (state == states.punch && sprite_index == spr_player_breakdanceuppercut))) || ((state == states.Sjump || state == states.Sjumpland) && (!(place_meeting(x, (y - 1), obj_destructibles))) && place_meeting(x, (y - 1), other))) && (!instance_exists(obj_fadeout)) && state != states.door && state != states.comingoutdoor)
        {
            obj_player1.lastroom = room
            obj_player2.lastroom = room
            scr_soundeffect(sfx_box)
...
if (place_meeting(x, (y + 1), obj_doorG) || place_meeting(x, (y - 1), obj_doorG))
    targetDoor = "G"
if (place_meeting(x, (y + 1), obj_doorH) || place_meeting(x, (y - 1), obj_doorH))
    targetDoor = "H"
if (place_meeting(x, (y + 1), obj_doorI) || place_meeting(x, (y - 1), obj_doorI))
    targetDoor = "I"
if (place_meeting(x, (y + 1), obj_doorJ) || place_meeting(x, (y - 1), obj_doorJ))
    targetDoor = "J"
if (place_meeting(x, (y + 1), obj_doorK) || place_meeting(x, (y - 1), obj_doorK))
    targetDoor = "K"
