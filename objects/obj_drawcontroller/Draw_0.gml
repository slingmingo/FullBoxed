...
{
    if (pizzashield == 1)
        draw_sprite_ext(spr_pizzashield, image_index, other.shieldlerp, y, xscale, scale_ys, 0, c_white, 1)
    if (state == states.titlescreen && sprite_index == spr_player_hyperpepintro && image_index < 23)
    {
        draw_sprite_ext(spr_pixel, 0, 0, 0, room_width, room_height, 0, c_black, obj_drawcontroller.hyperfade)
        if (obj_drawcontroller.hyperfade < 0.75)
            obj_drawcontroller.hyperfade += 0.0075
    }
}
...
with (obj_player1)
{
    if (visible && bbox_in_camera(view_camera[0], 32))
    {
        if (character != "D")
            ...
            pal_swap_set(spr_noisepalette, 14, 0)
        b = get_dark(image_blend, other.use_dark)
...
with (obj_baddie)
{
    ...
    if (visible && flash && bbox_in_camera(view_camera[0], 32))
        draw_sprite_ext(sprite_index, image_index, x, (y + _stun), (xscale * scale_xs), scale_ys, angle, image_blend, image_alpha)
}
with (obj_player)
{
    if (visible && flash && bbox_in_camera(view_camera[0], 32))
        draw_sprite_ext(sprite_index, image_index, x, y, (xscale * scale_xs), scale_ys, angle, image_blend, image_alpha)
}
