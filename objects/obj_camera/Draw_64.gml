...
if (obj_player.state != states.gameover || room != Titlescreen)
{
    ...
    if (room != Titlescreen)
    {
        ...
        if prankpossible
            ...
            rank_ix = 1
        if (_score >= global.srank && global.treasure == 1 && global.secretfound >= 3 && (!global.combodropped))
            prankpossible = 1
        else
            prankpossible = 0
        if (previousrank != rank_ix)
        {
            ...
            rank_scale = spr_entrancepillarsecret
        }
        ...
        var t = spr_h * perc
        var top = spr_h - t
        draw_sprite_ext((global.rank == "norank" ? spr_ranks_hud : spr_ranks_hud), rank_ix, rx, ry, rank_scale, rank_scale, spr_plug, c_white, 1)
        draw_sprite_part((global.rank == "norank" ? spr_ranks_hud : spr_ranks_hudfill), rank_ix, 0, top, spr_w, (spr_h - top), (rx - spr_xo), (ry - spr_yo + top))
...
if string_contains(room_get_name(room), "R")
    draw_sprite_ext(spr_pixel, 0, 0, 0, 960, 540, 0, c_maroon, 0.1)
if string_contains(room_get_name(room), "F")
    draw_sprite_ext(spr_pixel, 0, 0, 0, 960, 540, 0, c_white, 0.1)
