function scr_player_firemouth() //gml_Script_scr_player_firemouth
{
    ...
    if (global.slopemomentum == false)
    {
        if (scr_slope() && hsp != 0 && movespeed > 8)
            scr_player_addslopemomentumold(0.1, 0.25)
    }
    else if (global.slopemomentum == true)
    {
        if (scr_slope() && hsp != 0)
            scr_player_addslopemomentum(0.1, 0.25, 9)
    }
    ...
    if (sprite_index != spr_firemouthintro && sprite_index != spr_firemouthend)
    {
        ...
        else if (sprite_index != spr_firemouthdash)
        {
            ...
            if ((!doublejump) && key_slap2)
            {
                ...
                scr_soundeffect(sfx_firemouthdash)
                image_index = 0
                if (move != 0)
                    xscale = move
                dir = xscale
    ...
    else if (sprite_index == spr_firemouthjump)
    {
    ...
    if (sprite_index == spr_firemouthjump && (!ispeppino))
    {
        if (floor(image_index) == (image_number - 1))
            sprite_index = spr_playerN_firemouthfall
    }
    return;
}
