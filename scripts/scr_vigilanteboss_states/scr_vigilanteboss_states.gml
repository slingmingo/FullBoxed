    ...
    else if (combo_count > 0)
    {
        ...
        switch state
        {
            ...
            case states.dynamite:
                ...
                {
                    obj_explosion = obj_destroyablebigescape
                    image_xscale = other.image_xscale
                    movespeed = 6
                    vsp = -6
                }
...
function boss_vigilante_revolver() //gml_Script_boss_vigilante_revolver
{
    ...
    if (phase > 4)
        vigilante_cancel_attack(states.dynamite, 128)
    return;
}
...
function boss_vigilante_mach1() //gml_Script_boss_vigilante_mach1
{
    ...
    if (phase > 4)
        vigilante_cancel_attack(((!honor) ? states.revolver : states.handstandjump), 128)
    return;
}
...
function boss_vigilante_crouchslide() //gml_Script_boss_vigilante_crouchslide
{
    ...
    if (phase > 4)
        vigilante_cancel_attack(164, 128, states.uppunch)
    return;
}
...
function boss_vigilante_punch() //gml_Script_boss_vigilante_punch
{
    ...
    if (phase > 4)
        vigilante_cancel_attack(164, states.uppunch)
    return;
}
...
function boss_vigilante_superattack() //gml_Script_boss_vigilante_superattack
{
    ...
    with (lastplayerid)
    {
        if (state != 160 && state != states.hit && state != states.thrown && state != states.chainsaw)
        {
            state = 160
            x = room_width / 3
        }
