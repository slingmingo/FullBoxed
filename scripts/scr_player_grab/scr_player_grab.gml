function scr_player_grab() //gml_Script_scr_player_grab
{
    ...
    if grounded
    {
        ...
        if (move != 0)
        {
            ...
            else if (sprite_index != spr_swingding)
                image_speed = 0.35
        }
    }
    if (sprite_index != spr_swingding)
    ...
    else
    {
        ...
        hsp = xscale * movespeed
        if ((!key_attack) && scr_solid((x + xscale), y) && ((!(place_meeting((x + sign(hsp)), y, obj_slope))) || scr_solid_slope((x + sign(hsp)), y)) && (!(place_meeting((x + sign(hsp)), y, obj_destructibles))))
        {
            vsp = -4
        ...
        else if (scr_solid((x + xscale), y) && ((!(place_meeting((x + sign(hsp)), y, obj_slope))) || scr_solid_slope((x + sign(hsp)), y)) && (!(place_meeting((x + sign(hsp)), y, obj_destructibles))))
            xscale = (-xscale)
        with (instance_place((x + xscale), y, obj_destructibles))
            instance_destroy()
    }
    if (movespeed == 2 && sprite_index == spr_swingding && (!key_attack))
        sprite_index = spr_haulingidle
    if key_jump
        input_buffer_jump = 0
