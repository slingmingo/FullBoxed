function scr_collide_destructibles() //gml_Script_scr_collide_destructibles
{
    for (var i = 0; i < 2; i++)
    {
        ...
        with (_obj_player)
        {
            if ((state == states.jump && sprite_index == spr_playerN_noisebombspinjump) || state == states.skatespin || state == states.walljump || state == states.spin || state == 276 || state == states.hypersoar || state == states.slipbanan || state == states.rideweenie || state == states.trickjump || state == states.ratmountbounce || (state == states.pogo && pogochargeactive == 1))
            {
            ...
            if (state == states.freefall || state == states.freefallland || state == states.slipbanan || state == states.hypersoar)
            {
