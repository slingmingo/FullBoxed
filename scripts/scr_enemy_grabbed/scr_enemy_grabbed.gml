function scr_enemy_grabbed() //gml_Script_scr_enemy_grabbed
{
    if (!pepperman_grab)
    {
        ...
        {
            if (object_index != obj_grabbableblock)
            {
                x = _obj_player.x
        ...
        if (_obj_player.sprite_index == _obj_player.spr_swingding && _obj_player.hyperpeppino == 1)
        {
            depth = -7
            x = _obj_player.x + _obj_player.xscale * 55
            y = _obj_player.y
        }
        ...
        if (_obj_player.state == states.grab && _obj_player.sprite_index == _obj_player.spr_swingding && _obj_player.hyperpeppino == 0)
        {
            if (floor(_obj_player.image_index) == 0)
            {
                depth = -8
